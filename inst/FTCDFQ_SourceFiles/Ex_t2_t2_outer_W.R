...
dnsity <- function(x, mu, sigma, u){
  dex <- ifelse(x==0, T2f(Wtransf(Utransf(InvT2vec(u/(2*u+1)),sigma),mu)), 
       ifelse(x==1, 1 - T2f(Wtransf(Utransf(InvT2vec((u+1)/(2*u+1)),sigma),mu)), 
       (T2f(Wtransf(Utransf(InvT2vec((u+x)/(2*u+1) + 0.000001),sigma),mu)) - T2f(Wtransf(Utransf(InvT2vec((u+x)/(2*u+1)),sigma),mu)))/(0.000001*(2*u + 1))))
  return(dex)
  }
...
inverse_CDF <- function(x, mu, sigma, u){
  return((2*u+1)*(T2f(Uinv(Winv(InvT2f(x),sigma),mu)))-u)
}

# random draws function
rsamp <- function(n, mu, sigma, u){
  #  return (inverse_CDF(runif(n, min = 0, max = 1),mu,sigma, u))
  return((2*u+1)*(T2f(Uinv(Winv(InvT2vec(runif(n, min = 0, max = 1)),sigma),mu)))-u)
  }

# log_lik function
log_lik_Ex_t2_t2_outer_W <- function(i, prep) {
...
posterior_predict_Ex_t2_t2_outer_W <- function(i, prep, ...) {
  mu <- brms::get_dpar(prep, "mu", i = i)
  sigma <- brms::get_dpar(prep, "sigma", i = i)
  u <- brms::get_dpar(prep, "u", i = i)
  ndraws <- prep$ndraws
...
posterior_epred_Ex_t2_t2_outer_W <- function(prep) {
...
Ex_t2_t2_outer_W <- custom_family(
  "Ex_t2_t2_outer_W", dpars = c("mu", "sigma", "u"),
...
stan_funs <- '
  ...
  real Ex_t2_t2_outer_W_lpdf(real x, real mu, real sigma, real u){
    if(x==0){
      return log(T2(WTransform(U(InvT2(u/(2*u+1)),sigma),mu)));
    }
    if(x==1){
      return log(1 - T2(WTransform(U(InvT2((u+1)/(2*u+1)),sigma),mu)));
    }
    else{
       return log(T2(WTransform(U(InvT2((u+x)/(2*u+1)+0.00001),sigma),mu)) - T2(WTransform(U(InvT2((u+x)/(2*u+1)),sigma),mu))) - 
    log(2*u + 1);}
}'

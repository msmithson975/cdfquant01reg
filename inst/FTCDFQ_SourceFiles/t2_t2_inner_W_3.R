...
CDF <- function(x, mu, sigma, theta){
  dex <- ifelse(x==0, (sigma^2)*exp(theta), 
                ifelse(x==1, (sigma^2)*exp(-theta), 
                       (T2f(Utransf(Wtransf(InvT2f(x),theta),mu,sigma)))))
  return(dex)
}
...
dnsity <- function(x, mu, sigma, theta){
  dex <- ifelse(x==0, (sigma^2)*exp(theta), 
                ifelse(x==1, (sigma^2)*exp(-theta), 
                       (T2f(Utransf(Wtransf(InvT2f(x + 0.000001),theta),mu,sigma)) - T2f(Utransf(Wtransf(InvT2f(x),theta),mu,sigma)))/0.000001))
  return(dex)
}

inverse_CDF <- function(x, mu, sigma, theta){
  return(T2f(Winv(Uinv(InvT2f(x),mu,sigma),theta)))
}

# random draws function
rsamp <- function(n, mu, sigma, theta){
  return(T2f(Winv(Uinv(InvT2vec(runif(n, min = 0, max = 1)),mu,sigma),theta)))
}

# log_lik function
log_lik_t2_t2_inner_W_3 <- function(i, prep) {
...
posterior_predict_t2_t2_inner_W_3 <- function(i, prep, ...) {
  mu <- brms::get_dpar(prep, "mu", i = i)
  sigma <- brms::get_dpar(prep, "sigma", i = i)
  theta <- brms::get_dpar(prep, "theta", i = i)
  ndraws <- prep$ndraws
...
posterior_epred_t2_t2_inner_W_3 <- function(prep) {
...
t2_t2_inner_W_3 <- custom_family(
  "t2_t2_inner_W_3", dpars = c("mu", "sigma", "theta"),
...
stan_funs <- '
  ...
  real t2_t2_inner_W_3_lpdf(real x, real mu, real sigma, real theta){
    if(x==0){
    ...
    if(x==1){
      return log(pow(sigma,2)*exp(-theta));
       ...
       return log(T2(U(WTransform(InvT2(x+0.00001),theta),mu,sigma)) - T2(U(WTransform(InvT2(x),theta),mu,sigma)));}
}'

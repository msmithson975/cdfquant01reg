...
CDF <- function(x, mu, sigma, theta){
  ...
  if(x==1){
    ...
    return ((Cauchyf(Utransf(InvCauchyf(Vtransf(x,theta)),mu,sigma))))}
}
...
dnsity <- function(x, mu, sigma, theta){
  ...
  if(x==1){
    ...
    return ((Cauchyf(Utransf(InvCauchyf(Vtransf(x + 0.000001,theta)),mu,sigma)) - Cauchyf(Utransf(InvCauchyf(Vtransf(x,theta)),mu,sigma)))/0.000001)}
}

inverse_CDF <- function(x, mu, sigma, theta){
  return(Vinv(Cauchyf(Uinv(InvCauchyf(x),mu, sigma)),theta))
}
...
# log_lik function
log_lik_Cauchit_Cauchy_inner_V_3 <- function(i, prep) {
...
posterior_predict_Cauchit_Cauchy_inner_V_3 <- function(i, prep, ...) {
  mu <- brms::get_dpar(prep, "mu", i = i)
  sigma <- brms::get_dpar(prep, "sigma", i = i)
  theta <- brms::get_dpar(prep, "theta", i = i)
  ndraws <- prep$ndraws
...
posterior_epred_Cauchit_Cauchy_inner_V_3 <- function(prep) {
...
Cauchit_Cauchy_inner_V_3 <- custom_family(
  "Cauchit_Cauchy_inner_V_3", dpars = c("mu", "sigma", "theta"),
...
stan_funs <- '
  ...
  real Cauchit_Cauchy_inner_V_3_lpdf(real x, real mu, real sigma, real theta){
    if(x==0){
    ...
    if(x==1){
      return log(sigma*exp(-theta));
       ...
       return log(Cauchy(U(InvCauchy(VTransform(x+0.00001,theta)),mu,sigma)) - Cauchy(U(InvCauchy(VTransform(x,theta)),mu,sigma)));}
}'
...
my_family <- Cauchit_Cauchy_inner_V_3

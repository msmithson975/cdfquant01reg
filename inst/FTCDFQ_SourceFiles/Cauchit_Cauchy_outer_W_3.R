...
CDF <- function(x, mu, sigma, theta){
  ...
  if(x==1){
    ...
    return (Cauchyf(Wtransf(Utransf(InvCauchyf(x),mu,sigma),theta)))}
}
...
inverse_CDF <- function(x, mu, sigma, theta){
  return(Cauchyf(Uinv(Winv(InvCauchyf(x),theta),mu,sigma)))
}
...
# log_lik function
log_lik_Cauchit_Cauchy_outer_W_3 <- function(i, prep) {
...
posterior_predict_Cauchit_Cauchy_outer_W_3 <- function(i, prep, ...) {
  mu <- brms::get_dpar(prep, "mu", i = i)
  sigma <- brms::get_dpar(prep, "sigma", i = i)
  theta <- brms::get_dpar(prep, "theta", i = i)
  ndraws <- prep$ndraws
...
posterior_epred_Cauchit_Cauchy_outer_W_3 <- function(prep) {
...
Cauchit_Cauchy_outer_W_3 <- custom_family(
  "Cauchit_Cauchy_outer_W_3", dpars = c("mu", "sigma", "theta"),
...
stan_funs <- '
  ...
  if(x==0){
    ...
    return Cauchy(WTransform(U(InvCauchy(x),mu,sigma),theta));}
}
  
  real Cauchit_Cauchy_outer_W_3_lpdf(real x, real mu, real sigma, real theta){
    if(x==0){
...
my_family <- Cauchit_Cauchy_outer_W_3

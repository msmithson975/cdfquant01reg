...
CDF <- function(x, mu, sigma){
  ...
  if(x==1){
    ...
    return ((Cauchyf(Wtransf(Utransf(InvCauchyf(x),sigma),mu))))}
}
...
dnsity <- function(x, mu, sigma){
  ...
  if(x==1){
    ...
    return ((Cauchyf(Wtransf(Utransf(InvCauchyf(x + 0.000001),sigma),mu)) - Cauchyf(Wtransf(Utransf(InvCauchyf(x),sigma),mu)))/0.000001)}
}

inverse_CDF <- function(x, mu, sigma){
  return(Cauchyf(Uinv(Winv(InvCauchyf(x),sigma),mu)))
}
...
# log_lik function
log_lik_Cauchit_Cauchy_outer_W <- function(i, prep) {
...
posterior_predict_Cauchit_Cauchy_outer_W <- function(i, prep, ...) {
  mu <- brms::get_dpar(prep, "mu", i = i)
  sigma <- brms::get_dpar(prep, "sigma", i = i)
  ndraws <- prep$ndraws
...
posterior_epred_Cauchit_Cauchy_outer_W <- function(prep) {
...
Cauchit_Cauchy_outer_W <- custom_family(
  "Cauchit_Cauchy_outer_W", dpars = c("mu", "sigma"),
...
stan_funs <- '
  ...
  real Cauchit_Cauchy_outer_W_lpdf(real x, real mu, real sigma){
    if(x==0){
    ...
    if(x==1){
      return log(sigma*exp(-mu));
       ...
       return log(Cauchy(WTransform(U(InvCauchy(x+0.00001),sigma),mu)) - Cauchy(WTransform(U(InvCauchy(x),sigma),mu)));}
}'
...
my_family <- Cauchit_Cauchy_outer_W

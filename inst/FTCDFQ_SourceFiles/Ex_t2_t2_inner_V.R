...
dnsity2 <- function(x, mu, sigma, u){
  ...
  dens[x == 0] <- T2f(Utransf(InvT2vec(Vtransf(u[x == 0]/(2*u[x == 0]+1),mu[x == 0])),sigma[x == 0]))
  dens[x == 1] <- 1 - T2f(Utransf(InvT2vec(Vtransf((u[x == 1]+1)/(2*u[x == 1]+1),mu[x == 1])),sigma[x == 1]))
  idx <- (x > 0 & x < 1)
  dens[idx] <- (T2f(Utransf(InvT2vec(Vtransf((u[idx]+x[idx])/(2*u[idx]+1) + 0.000001,mu[idx])),sigma[idx])) - T2f(Utransf(InvT2vec(Vtransf((u[idx]+x[idx])/(2*u[idx]+1),mu[idx])),sigma[idx])))/(0.000001*(2*u[idx] + 1))
  return(dens)
}

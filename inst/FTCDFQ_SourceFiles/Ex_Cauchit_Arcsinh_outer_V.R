...
dnsity <- function(x, mu, sigma, u){
  if(x==0){
    return (Vtransf(Cauchyf(Utransf(InvArcsinhf(u/(2*u+1)),sigma)),mu))}
  if(x==1){
    return (1 - Vtransf(Cauchyf(Utransf(InvArcsinhf((u+1)/(2*u+1)),sigma)),mu))}
  else{
...
inverse_CDF <- function(x, mu, sigma, u){
  return((2*u+1)*(Arcsinhf(Uinv(InvCauchyf(Vinv(x,mu)),sigma)))-u)
}
...
# log_lik function
log_lik_Ex_Cauchit_Arcsinh_outer_V <- function(i, prep) {
...
posterior_predict_Ex_Cauchit_Arcsinh_outer_V <- function(i, prep, ...) {
  mu <- brms::get_dpar(prep, "mu", i = i)
  sigma <- brms::get_dpar(prep, "sigma", i = i)
  u <- brms::get_dpar(prep, "u", i = i)
  ndraws <- prep$ndraws
...
posterior_epred_Ex_Cauchit_Arcsinh_outer_V <- function(prep) {
...
Ex_Cauchit_Arcsinh_outer_V <- custom_family(
  "Ex_Cauchit_Arcsinh_outer_V", dpars = c("mu", "sigma", "u"),
...
stan_funs <- '
  ...
  real Ex_Cauchit_Arcsinh_outer_V_lpdf(real x, real mu, real sigma, real u){
    if(x==0){
      return log(VTransform(Cauchy(U(InvArcsinh(u/(2*u+1)),sigma)),mu));
    }
    if(x==1){
      return log(1 - VTransform(Cauchy(U(InvArcsinh((u+1)/(2*u+1)),sigma)),mu));
    }
    else{
       return log(VTransform(Cauchy(U(InvArcsinh((u+x)/(2*u+1)+0.00001),sigma)),mu) - VTransform(Cauchy(U(InvArcsinh((u+x)/(2*u+1)),sigma)),mu)) - 
    log(2*u + 1);}
}'
...
my_family <- Ex_Cauchit_Arcsinh_outer_V

...
CDF <- function(x, mu, sigma){
  dex <- ifelse(x==0, (sigma^2)*exp(mu), 
                ifelse(x==1, (sigma^2)*exp(-mu), 
                       (T2f(Utransf(Wtransf(InvT2f(x),mu),sigma)))))
  return(dex)
}
...
dnsity <- function(x, mu, sigma){
  dex <- ifelse(x==0, (sigma^2)*exp(mu), 
                ifelse(x==1, (sigma^2)*exp(-mu), 
                       (T2f(Utransf(Wtransf(InvT2f(x + 0.000001),mu),sigma)) - T2f(Utransf(Wtransf(InvT2f(x),mu),sigma)))/0.000001))
  return(dex)
}

inverse_CDF <- function(x, mu, sigma){
  return(T2f(Winv(Uinv(InvT2f(x),sigma),mu)))
}

# random draws function
rsamp <- function(n, mu, sigma){
  return(T2f(Winv(Uinv(InvT2vec(runif(n, min = 0, max = 1)),sigma),mu)))
}

# log_lik function
log_lik_t2_t2_inner_W <- function(i, prep) {
...
posterior_predict_t2_t2_inner_W <- function(i, prep, ...) {
  mu <- brms::get_dpar(prep, "mu", i = i)
  sigma <- brms::get_dpar(prep, "sigma", i = i)
  ndraws <- prep$ndraws
...
posterior_epred_t2_t2_inner_W <- function(prep) {
...
t2_t2_inner_W <- custom_family(
  "t2_t2_inner_W", dpars = c("mu", "sigma"),
...
stan_funs <- '
  ...
  real t2_t2_inner_W_lpdf(real x, real mu, real sigma){
    if(x==0){
    ...
    if(x==1){
      return log(pow(sigma,2)*exp(-mu));
       ...
       return log(T2(U(WTransform(InvT2(x+0.00001),mu),sigma)) - T2(U(WTransform(InvT2(x),mu),sigma)));}
}'

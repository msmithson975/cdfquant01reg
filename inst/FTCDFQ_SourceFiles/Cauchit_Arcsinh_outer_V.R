...
CDF <- function(x, mu, sigma){
  ...
  if(x==1){
    ...
    return ((Vtransf(Cauchyf(Utransf(InvArcsinhf(x),sigma)),mu)))}
}
...
dnsity <- function(x, mu, sigma){
  ...
  if(x==1){
    ...
    return ((Vtransf(Cauchyf(Utransf(InvArcsinhf(x + 0.000001),sigma)),mu) - Vtransf(Cauchyf(Utransf(InvArcsinhf(x),sigma)),mu))/0.000001)}
}

inverse_CDF <- function(x, mu, sigma){
  return(Arcsinhf(Uinv(InvCauchyf(Vinv(x,mu)),sigma)))
}
...
# log_lik function
log_lik_Cauchit_Arcsinh_outer_V <- function(i, prep) {
...
posterior_predict_Cauchit_Arcsinh_outer_V <- function(i, prep, ...) {
  mu <- brms::get_dpar(prep, "mu", i = i)
  sigma <- brms::get_dpar(prep, "sigma", i = i)
  ndraws <- prep$ndraws
...
posterior_epred_Cauchit_Arcsinh_outer_V <- function(prep) {
...
Cauchit_Arcsinh_outer_V <- custom_family(
  "Cauchit_Arcsinh_outer_V", dpars = c("mu", "sigma"),
...
stan_funs <- '
  ...
  real Cauchit_Arcsinh_outer_V_lpdf(real x, real mu, real sigma){
    if(x==0){
    ...
    if(x==1){
      return log((2*sigma*exp(-mu))/pi());
       ...
       return log(VTransform(Cauchy(U(InvArcsinh(x+0.00001),sigma)),mu) - VTransform(Cauchy(U(InvArcsinh(x),sigma)),mu));}
}'
...
my_family <- Cauchit_Arcsinh_outer_V

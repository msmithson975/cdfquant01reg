...
CDF <- function(x, mu, sigma){
  dex <- ifelse(x==0, (sigma^2)*exp(mu), 
                ifelse(x==1, (sigma^2)*exp(-mu), 
                       Vtransf(T2f(Utransf(InvT2vec(x),sigma)),mu)))
  return(dex)
}
...
dnsity <- function(x, mu, sigma){
  dex <- ifelse(x==0, (sigma^2)*exp(mu), 
                ifelse(x==1, (sigma^2)*exp(-mu), 
                       (Vtransf(T2f(Utransf(InvT2vec(x + 0.000001),sigma)),mu) - Vtransf(T2f(Utransf(InvT2vec(x),sigma)),mu))/0.000001))
  return(dex)
}

inverse_CDF <- function(x, mu, sigma){
  return(T2f(Uinv(InvT2vec(Vinv(x,mu)),sigma)))
}

# random draws function
rsamp <- function(n, mu, sigma){
  return(T2f(Uinv(InvT2vec(Vinv(runif(n, min = 0, max = 1),mu)),sigma)))}

# log_lik function
log_lik_t2_t2_outer_V <- function(i, prep) {
...
posterior_predict_t2_t2_outer_V <- function(i, prep, ...) {
  mu <- brms::get_dpar(prep, "mu", i = i)
  sigma <- brms::get_dpar(prep, "sigma", i = i)
  ndraws <- prep$ndraws
...
posterior_epred_t2_t2_outer_V <- function(prep) {
...
t2_t2_outer_V <- custom_family(
  "t2_t2_outer_V", dpars = c("mu", "sigma"),
...
stan_funs <- '
  ...
  real t2_t2_outer_V_lpdf(real x, real mu, real sigma){
    if(x==0){
    ...
    if(x==1){
      return log(pow(sigma,2)*exp(-mu));
       ...
       return log(VTransform(T2(U(InvT2(x+0.00001),sigma)),mu) - VTransform(T2(U(InvT2(x),sigma)),mu));}
}'

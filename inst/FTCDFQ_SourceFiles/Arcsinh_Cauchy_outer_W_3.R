...
CDF <- function(x, mu, sigma, theta){
  ...
  if(x==1){
    ...
    return ((Arcsinhf(Wtransf(Utransf(InvCauchyf(x),mu,sigma),theta))))}
}
...
# log_lik function
log_lik_Arcsinh_Cauchy_outer_W_3 <- function(i, prep) {
...
posterior_predict_Arcsinh_Cauchy_outer_W_3 <- function(i, prep, ...) {
  mu <- brms::get_dpar(prep, "mu", i = i)
  theta <- brms::get_dpar(prep, "theta", i = i)
  sigma <- brms::get_dpar(prep, "sigma", i = i)
  ndraws <- prep$ndraws
...
posterior_epred_Arcsinh_Cauchy_outer_W_3 <- function(prep) {
...
Arcsinh_Cauchy_outer_W_3 <- custom_family(
  "Arcsinh_Cauchy_outer_W_3", dpars = c("mu", "sigma", "theta"),
...
stan_funs <- '
  ...
  real Arcsinh_Cauchy_outer_W_3_lpdf(real x, real mu, real sigma, real theta){
    if(x==0){
    ...
    if(x==1){
      return log((pi()*sigma*exp(-theta))/2);
       ...
       return log(Arcsinh(WTransform(U(InvCauchy(x+0.00001),mu,sigma),theta)) - Arcsinh(WTransform(U(InvCauchy(x),mu,sigma),theta)));}
}'
...
my_family <- Arcsinh_Cauchy_outer_W_3

...
CDF <- function(x, mu, sigma, u){
  ifelse(x < -u,0, ifelse(x > 1+u,1, Arcsinhf(Utransf(Wtransf(InvArcsinhf((u+x)/(2*u+1)),mu),sigma))))}

inverse_CDF <- function(x, mu, sigma, u){
  return((2*u+1)*(Arcsinhf(Winv(Uinv(InvArcsinhf(x),sigma),mu)))-u)
}
...
# log_lik function
log_lik_Ex_Arcsinh_Arcsinh_inner_W <- function(i, prep) {
...
posterior_predict_Ex_Arcsinh_Arcsinh_inner_W <- function(i, prep, ...) {
  mu <- brms::get_dpar(prep, "mu", i = i)
  sigma <- brms::get_dpar(prep, "sigma", i = i)
  u <- brms::get_dpar(prep, "u", i = i)
  ndraws <- prep$ndraws
...
posterior_epred_Ex_Arcsinh_Arcsinh_inner_W <- function(prep) {
...
Ex_Arcsinh_Arcsinh_inner_W <- custom_family(
  "Ex_Arcsinh_Arcsinh_inner_W", dpars = c("mu", "sigma", "u"),
...
stan_funs <- '
  ...
  real Ex_Arcsinh_Arcsinh_inner_W_lpdf(real x, real mu, real sigma, real u){
    if(x==0){
      return log(Arcsinh(U(WTransform(InvArcsinh(u/(2*u+1)),mu),sigma)));
    }
    if(x==1){
      return log(1 - Arcsinh(U(WTransform(InvArcsinh((u+1)/(2*u+1)),mu),sigma)));
    }
    else{
       return log(Arcsinh(U(WTransform(InvArcsinh((u+x)/(2*u+1)+0.00001),mu),sigma)) - Arcsinh(U(WTransform(InvArcsinh((u+x)/(2*u+1)),mu),sigma))) - 
    log(2*u + 1);}
}'
...
my_family <- Ex_Arcsinh_Arcsinh_inner_W

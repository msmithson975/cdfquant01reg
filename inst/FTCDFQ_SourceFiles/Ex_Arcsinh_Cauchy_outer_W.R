...
dnsity2 <- function(x, mu, sigma, u){
  ...
  dens[x == 0] <- Arcsinhf(Wtransf(Utransf(InvCauchyf(u[x == 0]/(2*u[x == 0]+1)),sigma[x == 0]),mu[x == 0]))
  dens[x == 1] <- 1 - Arcsinhf(Wtransf(Utransf(InvCauchyf((u[x == 1]+1)/(2*u[x == 1]+1)),sigma[x == 1]),mu[x == 1]))
  idx <- (x > 0 & x < 1)
  dens[idx] <- (Arcsinhf(Wtransf(Utransf(InvCauchyf((u[idx]+x[idx])/(2*u[idx]+1) + 0.000001),sigma[idx]),mu[idx])) - Arcsinhf(Wtransf(Utransf(InvCauchyf((u[idx]+x[idx])/(2*u[idx]+1)),sigma[idx]),mu[idx])))/(0.000001*(2*u[idx] + 1))
  return(dens)
}

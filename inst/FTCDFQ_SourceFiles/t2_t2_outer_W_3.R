...
CDF <- function(x, mu, sigma, theta){
  ...
  if(x==1){
    ...
    return(T2f(Wtransf(Utransf(InvT2f(x),mu,sigma),theta)))
  }
}
...
inverse_CDF <- function(x, mu, sigma, theta){
  return(T2f(Uinv(Winv(InvT2f(x),theta),mu,sigma)))
}

# random draws function
rsamp <- function(n, mu, sigma, theta){
  return(T2f(Uinv(Winv(InvT2vec(runif(n, min = 0, max = 1)),theta),mu,sigma)))
}

# log_lik function
log_lik_t2_t2_outer_W_3 <- function(i, prep) {
...
posterior_predict_t2_t2_outer_W_3 <- function(i, prep, ...) {
  mu <- brms::get_dpar(prep, "mu", i = i)
  sigma <- brms::get_dpar(prep, "sigma", i = i)
  theta <- brms::get_dpar(prep, "theta", i = i)
  ndraws <- prep$ndraws
...
posterior_epred_t2_t2_outer_W_3 <- function(prep) {
...
t2_t2_outer_W_3 <- custom_family(
  "t2_t2_outer_W_3", dpars = c("mu", "sigma", "theta"),
...
stan_funs <- '
  ...
  real CDFF(real x, real mu, real sigma, real theta){
    if (x==0){return pow(sigma,2)*exp(theta);}
    if (x==1){return pow(sigma,2)*exp(-theta);}
    else{ 
    return T2(WTransform(U(InvT2(x),mu,sigma),theta));
    }
  }
  
  real t2_t2_outer_W_3_lpdf(real x, real mu, real sigma, real theta){

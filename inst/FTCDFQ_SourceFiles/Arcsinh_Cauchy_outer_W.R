...
CDF <- function(x, mu, sigma){
  ...
  if(x==1){
    ...
    return ((Arcsinhf(Wtransf(Utransf(InvCauchyf(x),sigma),mu))))}
}
...
# log_lik function
log_lik_Arcsinh_Cauchy_outer_W <- function(i, prep) {
...
posterior_predict_Arcsinh_Cauchy_outer_W <- function(i, prep, ...) {
  mu <- brms::get_dpar(prep, "mu", i = i)
  sigma <- brms::get_dpar(prep, "sigma", i = i)
  ndraws <- prep$ndraws
...
posterior_epred_Arcsinh_Cauchy_outer_W <- function(prep) {
...
Arcsinh_Cauchy_outer_W <- custom_family(
  "Arcsinh_Cauchy_outer_W", dpars = c("mu", "sigma"),
...
stan_funs <- '
  ...
  real Arcsinh_Cauchy_outer_W_lpdf(real x, real mu, real sigma){
    if(x==0){
    ...
    if(x==1){
      return log((pi()*sigma*exp(-mu))/2);
       ...
       return log(Arcsinh(WTransform(U(InvCauchy(x+0.00001),sigma),mu)) - Arcsinh(WTransform(U(InvCauchy(x),sigma),mu)));}
}'
...
my_family <- Arcsinh_Cauchy_outer_W

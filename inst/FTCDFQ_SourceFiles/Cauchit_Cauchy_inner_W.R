...
CDF <- function(x, mu, sigma){
  ...
  if(x==1){
    ...
    return ((Cauchyf(Utransf(Wtransf(InvCauchyf(x),mu),sigma))))}
}
...
dnsity <- function(x, mu, sigma){
  ...
  if(x==1){
    ...
    return ((Cauchyf(Utransf(Wtransf(InvCauchyf(x+0.000001),mu),sigma)) - Cauchyf(Utransf(Wtransf(InvCauchyf(x),mu),sigma)))/0.000001)}
}

inverse_CDF <- function(x, mu, sigma, u){
  return(Cauchyf(Winv(Uinv(InvCauchyf(x),sigma),mu)))
}
...
# log_lik function
log_lik_Cauchit_Cauchy_inner_W <- function(i, prep) {
...
posterior_predict_Cauchit_Cauchy_inner_W <- function(i, prep, ...) {
  mu <- brms::get_dpar(prep, "mu", i = i)
  sigma <- brms::get_dpar(prep, "sigma", i = i)
  ndraws <- prep$ndraws
...
posterior_epred_Cauchit_Cauchy_inner_W <- function(prep) {
...
Cauchit_Cauchy_inner_W <- custom_family(
  "Cauchit_Cauchy_inner_W", dpars = c("mu", "sigma"),
...
stan_funs <- '
  ...
  real Cauchit_Cauchy_inner_W_lpdf(real x, real mu, real sigma){
    if(x==0){
    ...
    if(x==1){
      return log(sigma*exp(-mu));
       ...
       return log(Cauchy(U(WTransform(InvCauchy(x+0.00001),mu),sigma)) - Cauchy(U(WTransform(InvCauchy(x),mu),sigma)));}
}'
...
my_family <- Cauchit_Cauchy_inner_W

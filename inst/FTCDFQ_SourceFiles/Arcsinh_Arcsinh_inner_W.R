...
CDF <- function(x, mu, sigma){
  ...
  if(x==1){
    ...
    return ((Arcsinhf(Utransf(Wtransf(InvArcsinhf(x),mu),sigma))))}
}
...
dnsity <- function(x, mu, sigma){
  ...
  if(x==1){
    ...
    return ((Arcsinhf(Utransf(Wtransf(InvArcsinhf(x+0.000001),mu),sigma)) - Arcsinhf(Utransf(Wtransf(InvArcsinhf(x),mu),sigma)))/0.000001)}
}

inverse_CDF <- function(x, mu, sigma){
  return(Arcsinhf(Winv(Uinv(InvArcsinhf(x),sigma),mu)))
}

# random draws function rArcsinh_Arcsinh_inner_W
rsamp <- function(n, mu, sigma){
  return (inverse_CDF(runif(n, min = 0, max = 1),mu,sigma)) }

# log_lik function
log_lik_Arcsinh_Arcsinh_inner_W <- function(i, prep) {
...
posterior_predict_Arcsinh_Arcsinh_inner_W <- function(i, prep, ...) {
  mu <- brms::get_dpar(prep, "mu", i = i)
  sigma <- brms::get_dpar(prep, "sigma", i = i)
  ndraws <- prep$ndraws
...
posterior_epred_Arcsinh_Arcsinh_inner_W <- function(prep) {
...
Arcsinh_Arcsinh_inner_W <- custom_family(
  "Arcsinh_Arcsinh_inner_W", dpars = c("mu", "sigma"),
...
stan_funs <- '
  ...
  real Arcsinh_Arcsinh_inner_W_lpdf(real x, real mu, real sigma){
    if(x==0){
      return log(sigma*exp(mu));}
    if(x==1){
      return log(sigma*exp(-mu));}
    else{
       return log(Arcsinh(U(WTransform(InvArcsinh(x+0.00001),mu),sigma)) - Arcsinh(U(WTransform(InvArcsinh(x),mu),sigma)));}
}'
...
my_family <- Arcsinh_Arcsinh_inner_W
